...
xPredictCompare <- function(list_pPerf, displayBy=c("ROC","PR"), type=c("curve","bar"), sort=TRUE, detail=TRUE, facet=FALSE, font.family="sans", signature=TRUE)
{
	...
	if(type=='curve'){
		...
		if(displayBy=='ROC'){
			...
			if(detail){
				p <- p + geom_line(aes(colour=factor(Label)), size=0.9)
			}else{
				p <- p + geom_line(aes(colour=factor(Method)))
			}
		
			p <- p + ylab("Sensitivity = TP/(TP+FN)") + xlab("1-Specificity = FP/(FP+TN)") + ylim(0,max(df_PRS$Recall)) + xlim(0,max(1-df_PRS$Specificity))
		...
		p <- p + theme_bw() + theme(legend.position="none",axis.title.y=element_blank(), axis.text.y=element_text(size=12,color="black"), axis.title.x=element_text(size=12,color="black")) + coord_flip()
