...
oUpsetAdv <- function(data, sortBy=c("auto","value","degree"), color="cyan4", shape=18, size=2, label.height.unit=NULL, member.levels=c("auto","num","customised"), levels.customised=NULL)
{
	...
	df %>% mutate(member=fct_inorder(member)) %>% ggplot(aes(x=member,y=value)) + geom_col(fill="steelblue", color='transparent', width=0.1, alpha=0.225) + geom_point(shape=shape, color=color, size=size, fill='white', alpha=1) + theme_classic() + theme(axis.title.x=element_blank()) -> gp
