...
xPredictROCR <- function(prediction, GSP, GSN, rescale=TRUE, plot=c("none","ROC","PR"), verbose=TRUE, font.family="sans", signature=TRUE)
{
    ...
    if(plot=='none'){
		...
		p <- p + geom_line() + theme_bw() + theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(), ,
		                                          axis.title.x = element_text(size = 12),
		                                          axis.title.y = element_text(size = 12)) + ylab("Precision = TP/(TP+FP)") + xlab("Recall = TP/(TP+FN)") + ylim(0,max(df_PRS$Precision)) + xlim(0,max(df_PRS$Recall)) 
		...
		p <- p + geom_point(data=df_PRS[i,], aes(x=Recall,y=Precision), colour="#0077B6")
		...
		p <- p + geom_text_repel(data=df_PRS[i,], family="Times New Roman", point.padding=2, box.padding=2, aes(x=Recall, y=Precision, label=paste0('Fmax = ',signif(fmax,digits=3))))
		...
		p <- p + geom_line() + theme_bw() + ylab("Sensitivity = TP/(TP+FN)") + xlab("1-Specificity = FP/(FP+TN)") + ylim(0,max(df_PRS$Recall)) + xlim(0,max(1-df_PRS$Specificity))
