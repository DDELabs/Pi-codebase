...
xEnrichDotplot <- function(eTerm, FDR.cutoff=0.05, colors=c("pink","red"), y.scale=c("normal","log"), slim=NULL, size.range=c(0.5,3.5), size.title="Num of overlaps", label.top='auto', label.direction.y=c("left","right","none"), label.size=2, ...)
{
	...
	gp <- gp + geom_point(aes(color=flag,size=nOverlap),alpha=0.5)
	gp <- gp + scale_colour_manual(values=colors) + guides(color="none")
	gp <- gp + xlab("Z-score") + ylab(expression(-log[10]("FDR")))
	gp <- gp + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.text.x = element_text(size = 6), axis.text.y = element_text(size = 6), axis.title.x = element_text(size = 8), axis.title.y = element_text(size = 8))
	...
	if(any(is.na(slim))){
		...
		gp <- gp + theme(legend.position="bottom", legend.key=element_rect(colour="transparent"), legend.title = element_text(size = 8), legend.text = element_text(size = 8)) + scale_size_continuous(limits=slim, range=size.range, guide=guide_legend(size.title,title.position="left",nrow=1))
	}
	...
	if(label.top=='auto'){
		...
		df <- df_enrichment_group %>% dplyr::group_by(group) %>% dplyr::top_n(zscore, n=label.top) %>% dplyr::filter(adjp<FDR.cutoff)
	}
	if(ngroup==1 & label.direction.y!='none'){
		...
		gp <- gp + ggrepel::geom_text_repel(data=df, aes(x=zscore,y=-log10(adjp),label=name), size=label.size, show.legend=F, segment.alpha=0.5, segment.color="grey50", segment.size=0.2, arrow=arrow(length=unit(0.00,'npc')), max.overlaps=Inf, direction="y", hjust=10, force=100,...)
	}
